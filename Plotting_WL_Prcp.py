...
def plot_prcp(df, rows, cols, savepath=None):
    fig, axes=plt.subplots(rows, cols, figsize=(cm2inch(15, 15)), layout='constrained')
    ...
    for i, column in enumerate(prcp.columns):
        ...
        ax.set_title(f'{column}', fontsize='xx-large')
        ax.tick_params(axis='x', labelsize='x-large')
        ax.tick_params(axis='y', labelsize='x-large')
        ...
        #set global y-axis label
        if column == '5190':
            ax.set_ylabel('Precipitation [mm/h]', fontsize='xx-large')
    ...
    legend_ax = fig.add_axes([0.907, 0.81, 0.1, 0.2])  # Adjust the coordinates and size as needed
    legend_ax.set_axis_off()
    legend_ax.legend(handles=legend_patches, loc='center', fontsize='x-large')
    plt.gcf().set_size_inches(cm2inch(50,20))
    if savepath:
        plt.savefig(savepath, dpi=600)
        plt.close()
    plt.show()
...
#     plot_water_level_bokeh(WL[[col]],  station_id_to_name.get(col))        
    ...
    savepath=r'C:\Users\henri\Documents\Universität\Masterthesis\Report\Prcp_all_stations.png'
    plot_prcp(prcp, 4, 3, savepath)
