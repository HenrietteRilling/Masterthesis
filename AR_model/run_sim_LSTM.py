...
forecast_horizons=[20, 1, 12, 24, 48, 168]  #1h, 1/2d, 1d, 2d, 1 week
test_horizons=[1, 12, 24, 48, 168,  672] #1h, 1d, 2d, 1 week, 1 month
nr_of_model_runs=5
epochs=150
...
window_size=10 #how many observations does the LSTM get

###################### Batch data ##########################################
#get input in batches with w timesteps 
features_train, labels_train = timeseries_dataset_from_array(X_train_sc, window_size, forecast_horizons[0], AR=True)
features_val, labels_val=timeseries_dataset_from_array(X_val_sc, window_size, forecast_horizons[0],AR=True) 
...
weightpath=os.path.join(respath, 'weights.pth')
if os.path.exists(weightpath): os.remove(weightpath)



model=sampleLSTM_AR(input_size, hidden_sizes[0], num_LSTM_layers, forecast_horizons[0])
...
#     respath=f'./Results/F1_FH_Uld_hs_{hs}'
#     if not os.path.exists(respath): os.makedirs(respath)
    
#     #initialize dataframes for saving performance metrics
#     rmse_df=pd.DataFrame(np.zeros((len(forecast_horizons), len(test_horizons))),index=forecast_horizons,columns=test_horizons)
#     PI_df=pd.DataFrame(np.zeros((len(forecast_horizons), len(test_horizons))),index=forecast_horizons,columns=test_horizons)
    
#     for fh in forecast_horizons:
#         rmse_all_model_runs=np.zeros((nr_of_model_runs, len(test_horizons)))
#         PI_all_model_runs=np.zeros((nr_of_model_runs, len(test_horizons)))
#         #train several times for each forecast horizon
#         for i in range(nr_of_model_runs):
#             print(f'\nModel {i} of forecast horizon {fh}') 
            
#             #delete any existing losslog/files, to only save losses of current model run
#             losslogpath=os.path.join(respath, f'losslog_{fh}_{i}.csv')
#             if os.path.exists(losslogpath): os.remove(losslogpath)
#             #delete any existing weights, only save weights of current model run
#             weightpath=os.path.join(respath, f'weights_{fh}_{i}.pth')
#             if os.path.exists(weightpath): os.remove(weightpath)
...
#             #plot losses
#             plot_losses(losslogpath, os.path.join(respath, f'losses_{fh}_{i}.png'))
...
#             for j, th in enumerate(test_horizons):
...
#                 # plot_test=np.concatenate(( preds_test_unsc[:,0], np.full(th-1, np.nan)))
            
#                 # Create a Bokeh figure for the last model
#                 if (i%nr_of_model_runs == 0) & (th==test_horizons[-1]):
#                     p = figure(x_axis_label='Date', y_axis_label='Water level [m]', title=f'Model {i} FH: {fh} TH: {th}')
                    
#                     #plot prediction
#                     p.line(pd.to_datetime(X_test.index)[1:-th+1], preds_test_unsc[:,0], line_width=2, legend_label='Prediction', line_color='darkorange')            
#                     # Plot observation
#                     p.line(pd.to_datetime(X_test.index)[1:-th+1], labels_test_unsc[:,0], line_width=2, legend_label='Observation', line_color='blue')
#                     # Customize the plot
...
#                     # Save the plot
#                     output_file(os.path.join(respath, f'preds_test_{i}_{fh}.html'))
#                     save(p)
        
        
#         #Calculate final metrics for each forecast horizon i.e., mean over all model runs
#         rmse_df.loc[fh][:]=rmse_all_model_runs.mean(axis=0)
#         PI_df.loc[fh][:]=PI_all_model_runs.mean(axis=0)
#         print(f'\nTrained and tested all models for forecast horizon: {fh}')
